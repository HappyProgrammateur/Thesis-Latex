...
For each query image, the most shared codebook dimensions with the best 15 remaining images and their respective patches were marked inside the images. It could be clearly seen in \figref{nn_query_search} that the patches with a comparable visual representation share the same cluster and (from a human perspective) seem to be representative for the chosen object class.
