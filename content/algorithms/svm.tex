...
To find a optimal separator, the equation \eqnref{svm:maxmarg} with the constraints $a_j \ge 0$ and $\sum_j a_j y_j = 0$ has to be solved. This quadratic programming optimization problem can be solved by a Lagrangian and the Kuhn-Tucker theorem as described by Cortes and Vapnik \cite{cortes1995support}.
...
As some data is not linear separable, it may is required to move the data into a higher dimensional space as shown in the sample images \figref{svm_nonsep}. In general a dataset of $N$ points can be linear separated in a $N-1$ dimensional space \cite{russellnorvig-ai}.
